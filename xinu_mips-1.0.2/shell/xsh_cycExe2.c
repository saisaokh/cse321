...
command xsh_cycExe2(ushort stdin, ushort stdout, ushort stderr, ushort nargs, char *args[])
{
  ...
  uchar test_array[THREAD_COUNT * STORE_COUNT];
   int a_tid, b_tid,c_tid,d_tid;

  int a_prio = 4, b_prio = 3, c_prio = 2,d_prio=1;
  int shared = 0;
  ...
  ready(d_tid = create(task4, 0, d_prio, "Task 4", 3, STORE_COUNT, test_array, &shared), RESCHED_NO);
